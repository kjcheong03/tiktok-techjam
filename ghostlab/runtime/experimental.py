...
class ExperimentalAgent:
    ...
    def _query_and_question(
        ...
        if self.question_variant == "none":
            ...
            question = "other"
        elif self.question_variant == "adaptive":
            question = None
        else:
            question = state.choose_question()
        if self.question_variant != "adaptive" and isinstance(state, ConversationState):
    ...
    def respond(
        ...
        if self.retrieval_route == "keyword":
            if self.field_sparse is None or self.sparse_weights is None:
                ...
                ranked = [
                    item.parent_asin
                    for item in self.field_sparse.search(
        ...
        question_reason = self.question_variant
    ...
    def override_last_question(self, session_id: str, attribute: str | None) -> None:
        ...
        self.sessions[session_id].last_asked_attribute = attribute
